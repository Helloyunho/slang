import { Position, Token, TokenType } from '../lexer/mod.ts'
...
export class AST {
  ...
  getReturnsValue(
    ...
    if (
      ...
      !skipTokenType.includes('FloatParsed') &&
      currentToken?.type === TokenType.Float &&
      upAST === undefined
    ) {
      // #region Float
      upAST = this.valueParsers.floatParser()
      // #endregion
    } else if (
      ...
      if (
        ...
        !skipTokenType.includes('ConditionStatement') &&
        currentToken.value === 'if' &&
        upAST === undefined
      ) {
        // #region If
        upAST = this.statements.conditionStatement()
        return upAST
        // #endregion
      } else if (
        !skipTokenType.includes('InitializeVariableStatement') &&
        ['const', 'let'].includes(currentToken.value) &&
        upAST === undefined
      ) {
        // #region InitializeVariable
        upAST = this.statements.initializeVariableStatement()
        return upAST
        // #endregion
      } else if (
        !skipTokenType.includes('TypeChangeExpression') &&
